...
public class Fight
{
		...
		public String getGmPacket()
		{
			...
			switch(_type)
			{
				...
				case 5://Perco
					...
					str+= "1;";//FIXME
					str+= "2;4;";//FIXME
					str+= (int)Math.floor(G.get_lvl()/2)+";"+(int)Math.floor(G.get_lvl()/2)+";"+(int)Math.floor(G.get_lvl()/2)+";"+(int)Math.floor(G.get_lvl()/2)+";"+(int)Math.floor(G.get_lvl()/2)+";"+(int)Math.floor(G.get_lvl()/2)+";"+(int)Math.floor(G.get_lvl()/2)+";";//Résistances
					str+= _team;
				break;
				case 10://Double
					str+= _double.get_classe()+";";
					str+= _double.get_gfxID()+"^"+_double.get_size()+";";
					str+= _double.get_sexe()+";";
					str+= _double.get_lvl()+";";
					str+= _double.get_align()+",";
					str+= "0,";//TODO
					str+= (_double.isShowingWings()?_double.getGrade():"0")+",";
					str+= _double.get_GUID()+";";
					str+= (_double.get_color1()==-1?"-1":Integer.toHexString(_double.get_color1()))+";";
					str+= (_double.get_color2()==-1?"-1":Integer.toHexString(_double.get_color2()))+";";
					str+= (_double.get_color3()==-1?"-1":Integer.toHexString(_double.get_color3()))+";";
					str+= _double.getGMStuffString()+";";
					str+= getPDV()+";";
					str+= getTotalStats().getEffect(Constants.STATS_ADD_PA)+";";
					str+= getTotalStats().getEffect(Constants.STATS_ADD_PM)+";";
					str+= getTotalStats().getEffect(Constants.STATS_ADD_RP_NEU)+";";
					...
					str+= getTotalStats().getEffect(Constants.STATS_ADD_MFLEE)+";";
					str+= _team+";";
					if(_double.isOnMount() && _double.getMount() != null)str+=_double.getMount().get_color();
					str+= ";";
				break;
			}
	...
	{	
		...
		_type = Constants.FIGHT_TYPE_PVT; //Perco
		_id = id;
		_map = map.getMapCopy();
		_mapOld = map;
		...
		SocketManager.GAME_SEND_GAME_ADDFLAG_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(),5,_init0.getGUID(),perco.getGuid(),(_init0.getPersonnage().get_curCell().getID()+1),"0;-1",perco.get_cellID(),"1;-1");
		SocketManager.GAME_SEND_ADD_IN_TEAM_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(),_init0.getGUID(), _init0);
	...
	public void leftFight(Personnage perso)
	{
		...
		if(Ancestra.CONFIG_DEBUG) GameServer.addToLog(perso.get_name()+" a quitter le combat");
		if(F != null)
		{
			if(_state >= Constants.FIGHT_STATE_ACTIVE)
			{
	...
	public boolean verifyStillInFight()	//Return true si au moins un joueur est encore dans le combat
	{
		for(Fighter f : _team0.values())
		{
			...
			&& f.getMob() != null
			&& f._Perco != null)	//Si c'est un groupe de monstre...
			{
				break;
			}
	...
	public void leftStatePlace(Fighter F, Personnage perso)
	{
		if(F.getTeam() == 0 && _team0.size() == 1 || F.getTeam() == 1 && _team1.size() == 1)
		{
			_state = Constants.FIGHT_STATE_FINISHED;
			this._init0.getPersonnage().get_curCarte().removeFight(this._id);
			SocketManager.GAME_SEND_FIGHT_GE_PACKET_TO_FIGHT(this,7,-1);
			for(Entry<Integer, Fighter> f : this._team0.entrySet())
			{
				if(f == null) continue;
				f.getValue().getPersonnage().teleport(perso.get_curCarte().get_id(), perso.get_curCell().getID());
				f.getValue().getPersonnage().set_duelID(-1);
				f.getValue().getPersonnage().set_ready(false);
				f.getValue().getPersonnage().fullPDV();
				f.getValue().getPersonnage().set_fight(null);
				f.getValue().getPersonnage().refreshMapAfterFight();
			}
		...
		}else
		{
			...
			SocketManager.GAME_SEND_FIGHT_GE_PACKET_TO_FIGHT(this,7,-1);
			SocketManager.GAME_SEND_ERASE_ON_MAP_TO_FIGHT(this, F.getGUID());
			perso.set_duelID(-1);
			perso.set_ready(false);
			perso.fullPDV();
			perso.set_fight(null);
			//On replace le joueur sur le world.
			perso.teleport(perso.get_curCarte().get_id(), perso.get_curCell().getID());
			F.getPersonnage().refreshMapAfterFight();
		}
	}
	...
	public static void FightStateAddFlag(Carte _map, Personnage P)
	{
		for(Entry<Integer, Fight> fight : _map.get_fights().entrySet())
		{
			...
			{
				if(fight.getValue()._type == Constants.FIGHT_TYPE_CHALLENGE || fight.getValue()._type == Constants.FIGHT_TYPE_AGRESSION)
				{
